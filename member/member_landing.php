<?php
// member_landing.php - Member Landing Page with Manual + QR Check-in
...
// Handle Check-in (manual or QR)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['checkin'])) {
    $member_id_input = trim($_POST['member_id']);

    // Find member (by member_id) and active
    ...
    if ($member) {
        ...
        $stmt = $conn->prepare("SELECT * FROM attendance WHERE member_id = ? AND date = CURDATE() AND check_in_time IS NOT NULL");
        ...
        $stmt = $conn->prepare("SELECT * FROM subscriptions WHERE member_id = ? AND status = 'Active' AND start_date <= CURDATE() AND end_date >= CURDATE()");
        $stmt->execute([$member_id]);
        ...
        if (!$subscription) {
            ...
            $success = "Check-in successful! Redirecting...";
            header("refresh:1;url=member_dashboard.php");
        }
    } else {
...
.checkin-card { background:white; border-radius:20px; padding:40px; box-shadow:0 15px 35px rgba(0,0,0,0.1); max-width:500px; width:100%; }
.logo { text-align:center; margin-bottom:30px; }
.logo i { font-size:4rem; color: var(--primary-color); }
.checkin-input { font-size:1.5rem; text-align:center; border:3px solid #e9ecef; border-radius:10px; padding:15px; margin-bottom:20px; }
.checkin-input:focus { border-color: var(--primary-color); box-shadow:0 0 0 0.2rem rgba(255,107,107,0.25); }
.btn-checkin { background: linear-gradient(135deg, var(--primary-color) 0%, #ff8e8e 100%); border:none; padding:15px; font-size:1.2rem; font-weight:bold; border-radius:10px; width:100%; }
.btn-checkin:hover { background: linear-gradient(135deg, #ff8e8e 0%, var(--primary-color) 100%); }
        ...
        <div class="col-md-6">
            <div class="checkin-card">
                ...
                <form method="POST" id="manualForm">
                    <input type="text" class="form-control checkin-input" name="member_id" placeholder="Enter Member ID" required autofocus>
                    <button type="submit" name="checkin" class="btn btn-checkin mt-2"><i class="fas fa-sign-in-alt me-2"></i>CHECK IN</button>
                </form>
    ...
    html5QrCode.start(
        ...
        (decodedText) => {
            fetch('member_landing.php', {
                ...
                body: `checkin=1&member_id=${encodeURIComponent(decodedText)}`
